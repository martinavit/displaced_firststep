...
//==================================================================
void Analysis_1file::analisi(int num_histo_kin
                          ){
  ...
  const int nSamples_signal=1;
    

  const TString fileList[nSamples] = {  "gev2.root",
  ...
  const TString catNames[nCat] ={"all","ossf", "no_ossf", "3tracks", "2tracks", "1track","nada"};
  ...
  for(int sam = 0, effsam = 0; sam < nSamples; ++sam, ++effsam){
	...
	printProgress(progress);
      ...
      //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<            
      //------------------------------------------------------------ selection class
      for(int v =0; v < _nVFit; v++){
	for (int i =0; i < 12; i++){
	  cout<<i<<")  "<<v<<"]  "<<_vertices[v][i]<<endl;
	}
      }
	...
	if (_vertices[0][v] == iV_lt) {
      ...
      //  cout<< _vertex_X[0]<<"  "<< _vertex_X[1]<< "   "<< _vertex_X[2]  <<endl;    
      */
      //--------------------------------------------------------------------------------   
	...
	if (_2tracks)         Histos[numero_histo][4][fill]->Fill(TMath::Min(values[numero_histo], maxBinC[numero_histo]), scal);
       	if (_1track)          Histos[numero_histo][5][fill]->Fill(TMath::Min(values[numero_histo], maxBinC[numero_histo]), scal);	
      }// end for histo
...
//==================================================================
double Analysis_1file::maximum(double a, double b){
  double massimo=0;
  massimo = a;
...
double Analysis_1file::derivateR(double a, double x, double y, double z){
  double result =0;
  result = a /((x*x + y*y + z*z)*(x*x + y*y + z*z));
  return result;
}
//___________________________________________________________________
double Analysis_1file::derivate2_with_sigmaR(double a, double sa, double x, double y, double z){
  double result =0;
  result = (a*a)*(sa*sa) /(x*x + y*y + z*z);
  return result;
}
//___________________________________________________________________
double Analysis_1file::derivateR2D(double a, double x, double y){
  double result =0;
  result = a /((x*x + y*y )*(x*x + y*y ));
  return result;
}
//___________________________________________________________________
double Analysis_1file::derivate2_with_sigmaR2D(double a, double sa, double x, double y){
  double result =0;
...
void Analysis_1file::from_TGraph_to_TH1D (TGraphAsymmErrors *graph, TH1D *histo, int number_point){
...
/*
//==================================================================
double Analysis_1file::fakeWeight(const unsigned ind, const int flavors, const double conePt, const double eta, const bool tight, TH2D* frMap, const unsigned lCount){
unsigned nFO = 0;
double fr[4];
for(unsigned l = 0; l < lCount; ++l){
if(!tight[ind[l]]){
...
//==================================================================
double Analysis_1file::FR_factor(TGraphAsymmErrors *fakeRate_mu[3],
...
//___________________________________________________________________
void Analysis_1file::class_os(int event_clas[1], int  flavors_3l[3], int  charge_3l[3]){
...
//___________________________________________________________________
void Analysis_1file::ossf_no_ossf(int kind[1],TLorentzVector pair[3],TLorentzVector leep1, TLorentzVector leep2,TLorentzVector leep3, int  flavors_3l[3], int  charge_3l[3]){
...
//___________________________________________________________________
void Analysis_1file::fr_selection(int number, TLorentzVector lepton_fake_order[3],TLorentzVector leep1, TLorentzVector leep2,TLorentzVector leep3, int index_leptons[3],  int flavor_leptons[3], int origin_leptons[3],int index_3l[3],  int flavor_3l[3], int origin_3l[3]){
